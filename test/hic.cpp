...
void Hic(int argc, char* argv[]){
	...
	string tmpline, ele1, ele2, ele3, ele4, ele5;

	while(getline(input, tmpline)){
		stringstream ss;
		ss << tmpline;
		ss >> ele1 >> ele2 >> ele3>>ele4 >> ele5 ;

		// start position, end position, and the contact frequency
		int start, end, freq;

		stringstream ss2, ss3, ss5;
		ss2 << ele2; ss2 >> start;
		ss3 << ele3; ss3 >> end;
		ss5 << ele5; ss5 >> freq;
		...
		matrix[start][end] += freq;
  		matrix[end][start] += freq;
